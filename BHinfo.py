...
for rarity, info in rarity_coef.iteritems():
  step = 1.0;
  denominator = 1.0;
  for i in xrange(0, info["step"]):
    step *= rarity_step[i];
    denominator = 1 + denominator * rarity_step[i];
  info["coef"] = []
  info["coef"].append([[1.0, 0], [1 - step/denominator, 1.0/ denominator]]);
  for i in xrange(0, info["step"]):
    info["coef"].append([[info["coef"][i][1][0] * rarity_step[i], info["coef"][i][1][1] * rarity_step[i]],[info["coef"][i][1][0] * rarity_step[i] - step/denominator, info["coef"][i][1][1] * rarity_step[i] + 1.0/denominator]]);
